...
#' @examples
#' \donttest{
#' covid.svy <- get_covid_httr2(year=2020, month=5, vars=c("C001","C002"),
...
get_covid_httr2 <- function(year, month, vars = NULL,
                       labels = TRUE, deflator = TRUE, design = TRUE, reload = TRUE, savedir = tempdir())
{
  ...
  if (labels == TRUE) {
    if (exists("covid_labeller", where="package:COVIDIBGE", mode="function")) {
      dicfiles <- unlist(strsplit(unlist(strsplit(unlist(strsplit(gsub("\r\n", "\n", httr2::req_perform(req=httr2::request(base_url=ftpdoc)) |> httr2::resp_body_string()), "\n")), "<a href=[[:punct:]]")), ".xls"))
      if (month < 10) {
      ...
      if (reload == FALSE & file.exists(paste0(savedir, "/", dicname))) {
        ...
        httr2::req_perform(req=httr2::request(base_url=paste0(ftpdoc, dicname)) |> httr2::req_progress(), path=paste0(savedir, "/", dicname), verbosity=1)
        if (reload == FALSE) {
  ...
  if (deflator == TRUE) {
    if (exists("covid_deflator", where="package:COVIDIBGE", mode="function")) {
      ...
      if (reload == FALSE & file.exists(paste0(savedir, "/Deflatores.zip"))) {
        ...
        httr2::req_perform(req=httr2::request(base_url=paste0(ftpdoc, defzip)) |> httr2::req_progress(), path=paste0(savedir, "/Deflatores.zip"), verbosity=1)
        if (reload == FALSE) {
